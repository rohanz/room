...
- The plugin manifests and marketplace entry carry the release version (0.16.0); `package.json` versions at 0.1.0 are private workspace package versions, not the plugin release version. The MCP handshake reads the Claude plugin manifest version.
