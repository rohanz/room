...
export async function dropSatisfiedBaseNotice(s: Recipient, m: Msg, timeoutMs?: number): Promise<boolean> {
  if (m.type !== 'base') return false
  if (timeoutMs !== undefined && timeoutMs <= 0) return false
  try {
    await git(s.dir, ['merge-base', '--is-ancestor', m.base, 'HEAD'], timeoutMs)
