...
it('reads only hook and explicit worker runtime metadata, and clears absent values', () => {
  ...
  expect(resolveSessionRuntime(dir, { ...worker, ROOM_WORKER_ID: 'worker-1' })).toEqual({ model: 'gpt-6-astra', effort: 'medium' })
  expect(resolveSessionRuntime(dir, { ROOM_WORKER_ID: 'worker-2' })).toEqual({ model: undefined, effort: undefined })
  fs.writeFileSync(sessionMetadataPath(dir), JSON.stringify({ worker_id: 'worker-1', model: 'actual', effort: 'high' }))
  expect(resolveSessionRuntime(dir, { ...worker, ROOM_WORKER_ID: 'worker-1' })).toEqual({ model: 'actual', effort: 'high' })
  expect(resolveSessionRuntime(dir, {})).toEqual({ model: undefined, effort: undefined })
