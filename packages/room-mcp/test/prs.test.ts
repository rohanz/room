...
describe('ledger to PR', () => {
  ...
  it('renderPrNote tells the story in bus order with plan outcomes and answers', () => {
    ...
    const iScope = idx(/alice\*\* is on `auth`: sessions \(`src\/auth\.py`\)/)
    const iClaim = idx(/claimed `src\/auth\.py:1-5` — rename login; plans: rename login → sign_in → done: renamed login to sign_in/)
    const iQ = idx(/bob\*\* asked alice: keep the old name\?/)
    const iA = idx(/^  - .*alice\*\* answered: no, sign_in only/)
    ...
    const iDone = idx(/alice\*\* done \(auth\): sign_in landed, 12 tests pass/)
    ...
    expect(md).toContain('- alice: `src/auth.py:9-9` — still working')
  })
