...
describe('Claude wake guidance', () => {
  ...
  it('is silent for channels, explicit opt-out, and Codex', () => {
    expect(claudeWakeNote(session(), 'spawn', { host: 'claude', parentArgs: admittedClaude })).toBe('')
    vi.stubEnv('ROOM_CLAUDE_CHANNEL', '')
    expect(claudeWakeNote(session(), 'spawn', { host: 'claude', parentArgs: plainClaude })).toBe('')
  ...
  it('prints a copyable alias for the current shell, defaulting to zsh', () => {
    for (const [shell, rc] of [['/bin/zsh', '~/.zshrc'], ['/bin/bash', '~/.bashrc'], [undefined, '~/.zshrc']] as const) {
      ...
      expect(note.split('\n')[1]).toBe(`To turn wake-ups on, start Claude Code with \`claude-room\`. If that command is not found, add it: \`echo "alias claude-room='claude --dangerously-load-development-channels plugin:room@room'" >> ${rc}\``)
