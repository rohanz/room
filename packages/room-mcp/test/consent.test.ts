...
it.each(['full', 'declared', 'intent'] as ShareLevel[])('discloses %s on the first solo team join once', async share => {
  ...
  expect(out).toContain('to keep file contents on this machine, say: share plans only')
  expect(out).toContain('to share only my declared files, say: only my declared files')
  expect(out).not.toContain('room_share')
