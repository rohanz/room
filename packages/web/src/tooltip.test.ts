...
it('aligns a full-width code line tooltip to the pane right edge and below the line', () => {
  Object.defineProperty(window, 'innerWidth', { value: 1440 })
  const pane = document.createElement('div'); pane.className = 'code-scroll'
  trigger.replaceWith(pane); pane.append(trigger)
  vi.spyOn(pane, 'getBoundingClientRect').mockReturnValue(rect(100, 0, 1000, 700))
  vi.mocked(trigger.getBoundingClientRect).mockReturnValue(rect(100, 290, 1000, 20))
  bindTooltip(trigger, 'Line details', 'code-right')
  trigger.dispatchEvent(new dom.window.Event('pointerenter'))
  expect(Number.parseFloat(overlay().style.left) + overlay().offsetWidth).toBeCloseTo(1100, 0)
  expect(overlay().style.top).toBe('318px')
})
it('code placement flips above near the viewport bottom and keyboard focus uses it too', () => {
  const pane = document.createElement('div'); pane.className = 'code-scroll'
  trigger.replaceWith(pane); pane.append(trigger)
  vi.spyOn(pane, 'getBoundingClientRect').mockReturnValue(rect(100, 0, 800, 768))
  vi.mocked(trigger.getBoundingClientRect).mockReturnValue(rect(100, 740, 800, 20))
  bindTooltip(trigger, 'Details', 'code-right')
  trigger.focus()
  expect(overlay().style.left).toBe('700px')
  expect(overlay().style.top).toBe('632px')
})
it('keeps Board-style element tooltips below-left', () => {
  bindTooltip(trigger, 'Board details')
  trigger.dispatchEvent(new dom.window.Event('pointerenter'))
