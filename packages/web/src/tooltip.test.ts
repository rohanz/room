...
beforeEach(() => {
  ...
  vi.spyOn(dom.window.HTMLElement.prototype, 'getBoundingClientRect').mockReturnValue(rect(0, 0, 200, 100))
})
...
it('portals outside clipping ancestors, positions below-right, and reuses one root', () => {
  showTooltip(trigger, 'First')
  expect(overlay().parentElement).toBe(document.body)
  expect(overlay().style.left).toBe('148px'); expect(overlay().style.top).toBe('138px')
...
it('flips above and left at viewport edges, follows nested scroll and resize, and clamps to 8px', () => {
  vi.mocked(trigger.getBoundingClientRect).mockReturnValue(rect(980, 740, 40, 20))
  showTooltip(trigger, 'Edge')
  expect(overlay().style.left).toBe('772px'); expect(overlay().style.top).toBe('632px')
  ...
  expect(overlay().style.left).toBe('248px'); expect(overlay().style.top).toBe('228px')
  vi.mocked(trigger.getBoundingClientRect).mockReturnValue(rect(-30, -30, 10, 10))
  window.dispatchEvent(new dom.window.Event('resize'))
  expect(overlay().style.left).toBe('8px'); expect(overlay().style.top).toBe('8px')
})
