...
describe('identity', () => {
  ...
  it('older identities without owner/kind extras use the same participant name', () => {
  ...
  it('describeIdentity is the participant-list line', () => {
    ...
    expect(describeIdentity({ name: 'rohanz', kind: 'agent', owner: 'rohanz' })).toBe("rohanz's agent")
    expect(describeIdentity({ name: 'rohanz', kind: 'agent', owner: 'rohanz', label: 'main' })).toBe("rohanz's agent · main")
    expect(describeIdentity({ name: 'deploy', kind: 'bot', owner: 'rohanz' })).toBe('deploy [bot] · bot of rohanz')
  })
