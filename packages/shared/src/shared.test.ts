...
describe('identity', () => {
  ...
  it('assigns distinct join-order colours, retains them, and repairs concurrent collisions', () => {
    ...
    a.assignColor('Ann'); b.assignColor('Bob')
    const aLocal: string[] = [], bLocal: string[] = []
    a.colors.observe((event, transaction) => { if (transaction.local) aLocal.push(...event.keysChanged) })
    b.colors.observe((event, transaction) => { if (transaction.local) bLocal.push(...event.keysChanged) })
    const updateA = Y.encodeStateAsUpdate(aDoc), updateB = Y.encodeStateAsUpdate(bDoc)
    const bobBeforeMerge = Y.encodeStateVector(bDoc)
    Y.applyUpdate(aDoc, updateB); Y.applyUpdate(bDoc, updateA)
    Y.applyUpdate(aDoc, Y.encodeStateAsUpdate(bDoc, bobBeforeMerge))
    expect([...a.colors.entries()].sort()).toEqual([['Ann', 0], ['Bob', 1]])
    expect([...b.colors.entries()].sort()).toEqual([['Ann', 0], ['Bob', 1]])
    expect(aLocal).toEqual([])
    expect(bLocal).toEqual(['Bob'])
