---
name: room-join
description: Join the shared room for this repo so this Codex session can see what teammates and their agents are doing and coordinate with them. Use when the user says "join the room", "$room-join", or asks to work alongside a teammate's agent.
---

Call `room_join` with no arguments. It derives the room from the git origin and branch,
takes your name from `git config user.name`, starts the push-only sync daemon, and returns
who is here, their scopes, open claims, and the browser view URL. Nothing is written to
disk by the room.
