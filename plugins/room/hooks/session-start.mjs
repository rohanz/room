...
if (root && id) {
  ...
  try { fs.writeFileSync(path.join(stateDir, 'room-session.json'), JSON.stringify({ session_id: id, at: Date.now(), cwd: ev.cwd, host, ...(process.env.ROOM_WORKER_ID ? { worker_id: process.env.ROOM_WORKER_ID } : {}), ...(host === 'claude' && typeof ev.transcript_path === 'string' && ev.transcript_path ? { transcript_path: ev.transcript_path } : {}), ...(typeof ev.model === 'string' && ev.model.trim() ? { model: ev.model.trim().slice(0, 80), modelFromHook: true } : {}), ...(typeof ev.effort?.level === 'string' && ev.effort.level.trim() ? { effort: ev.effort.level.trim().slice(0, 80) } : {}) }) + '\n') } catch { /* best effort */ }
} else {
