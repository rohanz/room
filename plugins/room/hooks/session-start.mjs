...
import { readStdinJson, gitRoot, gitStatePath, readJson, readHookSeen, writeHookSeen, companyLine } from './common.mjs'
...
if (root && id) {
  try { fs.writeFileSync(gitStatePath(root, 'room-hook-activity.json'), JSON.stringify({ session_id: id, at: Date.now() })) } catch { /* best effort */ }
