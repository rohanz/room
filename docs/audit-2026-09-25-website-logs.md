...
1. **Notes from a lead's own workers wake it constantly (NEW) — FIXED in 0.16.0 (COMMIT).** 65 wakes in 28 minutes from 6
...
2. **room_wait's 120 s cap equals Claude Code's move-to-background point (NEW) — FIXED in 0.16.0 (COMMIT).** 33 of 154 waits
...
3. **A stale pre-fix server froze the session — FIXED in 0.16.0 (COMMIT) for
...
4. **An old Room process on the same clone appears as another person (phantom-self part NEW) — FIXED in 0.16.0 (COMMIT).**
   ...
   **NOT DONE:** `room_read` of one's own file can still select an older same-checkout
   session as publisher.
5. **Log spam and idle CPU on a large repo (NEW) — FIXED in 0.16.0 (COMMIT).** ~7,700 "skip <ignored file>" lines in 13
...
6. **Collect keeps worktrees over regenerable build output (on the roadmap, happens every time) — FIXED in 0.16.0 (COMMIT).**
...
7. **Collect is slow, serial, and misreports queueing (NEW) — FIXED in 0.16.0 (COMMIT) for speed and queue reporting; collection remains serialized.** 60–130 s per collect (median 23 s);
   three in parallel took 66 s, 130 s, then "error: room not synced yet, retry" at 131 s; the retry
   worked.
8. **Collect and discard trip on stale records (NEW) — FIXED in 0.16.0 (COMMIT).** One vanished worktree (`ENOENT lstat
...
9. **Worker processes outlive their worker (NEW) — FIXED in 0.16.0 (COMMIT).** A worker's `astro dev` from 22 Sep is still
...
10. **Claims and overlays of finished or dead workers linger (partly fixed in 0.14.1) — FIXED in 0.16.0 (COMMIT).** Two workers
...
11. **Merge preview unusable here (general item on the roadmap) — FIXED in 0.16.0 (COMMIT) for preview without `run`; preview with `run` still materializes the full tree.** The lead broadcast "skip
    room_preview_merge" (slow with large untracked art) and never ran one. That ruling, and another,
    went out as `fyi`.
12. **Carry friction (NEW) — FIXED in 0.16.0 (COMMIT).** The lead stashed the human's tracked edits before spawning because
...
13. **Smaller — partly FIXED in 0.16.0 (COMMIT).** 5 spawns "not in a room" after a restart although the local room was remembered,
    ...
    alias, and question/answer surfacing are fixed. **NOT DONE:** the possessive
    "rohanz+claude's agent" label has no reported change.
...
- FIXED in 0.16.0 (COMMIT): room_preview_merge refuses the lead's OWN `share=intent` worker ("shares intent only"); collect works.
- FIXED in 0.16.0 (COMMIT): A worker gave up after "three unanswered waits" although the answer had been sent (the 0.15.2
  batch lead saw the same crossing).
- FIXED in 0.16.0 (COMMIT): room_state keeps counting collected and discarded workers as finished.
- FIXED in 0.16.0 (COMMIT): The discard recovery patch is named with the UTC date.
- FIXED in 0.16.0 (COMMIT): `archive-signal.test.ts` ("early tar exit while git archive is piping") hung once on CI's Linux
