...
1. **Notes from a lead's own workers wake it constantly (NEW) — FIXED in 0.16.0 (e5f8309).** 65 wakes in 28 minutes from 6
...
2. **room_wait's 120 s cap equals Claude Code's move-to-background point (NEW) — FIXED in 0.16.0 (e5f8309).** 33 of 154 waits
...
3. **A stale pre-fix server froze the session — FIXED in 0.16.0 (7274f39) for
   cancellation and the stale-code warning; root cause fixed in 0.15.1.** Six spawns backgrounded at 165–721 s, room_state took 121 s.
...
4. **An old Room process on the same clone appears as another person (phantom-self part NEW) — FIXED in 0.16.0 (7274f39, 7ce33e1).**
   ...
   A session that is not its checkout's publisher now reads its own files from disk (7ce33e1).
5. **Log spam and idle CPU on a large repo (NEW) — FIXED in 0.16.0 (889a1b8).** ~7,700 "skip <ignored file>" lines in 13
...
6. **Collect keeps worktrees over regenerable build output (on the roadmap, happens every time) — FIXED in 0.16.0 (d2f87a2).**
...
7. **Collect is slow, serial, and misreports queueing (NEW) — FIXED in 0.16.0 (d2f87a2, 889a1b8) for speed and queue reporting; collection remains serialized.** 60–130 s per collect (median 23 s);
   three in parallel took 66 s, 130 s, then "error: room not synced yet, retry" at 131 s; the retry
   worked.
8. **Collect and discard trip on stale records (NEW) — FIXED in 0.16.0 (d2f87a2).** One vanished worktree (`ENOENT lstat
...
9. **Worker processes outlive their worker (NEW) — FIXED in 0.16.0 (58bd487, bf49456).** A worker's `astro dev` from 22 Sep is still
...
10. **Claims and overlays of finished or dead workers linger (partly fixed in 0.14.1) — FIXED in 0.16.0 (58bd487).** Two workers
...
11. **Merge preview unusable here (general item on the roadmap) — FIXED in 0.16.0 (889a1b8) for preview without `run`; preview with `run` still materializes the full tree.** The lead broadcast "skip
    room_preview_merge" (slow with large untracked art) and never ran one. That ruling, and another,
    went out as `fyi`.
12. **Carry friction (NEW) — FIXED in 0.16.0 (d2f87a2).** The lead stashed the human's tracked edits before spawning because
...
13. **Smaller — partly FIXED in 0.16.0 (e5f8309, 7274f39).** 5 spawns "not in a room" after a restart although the local room was remembered,
    ...
    piggybacked on unrelated collect or spawn results. Remembered local rejoin, `message`
    alias, and question/answer surfacing are fixed. Tagged names now print as
    `rohanz+claude`, not "rohanz+claude's agent" (7274f39).
...
- FIXED in 0.16.0 (58bd487): room_preview_merge refuses the lead's OWN `share=intent` worker ("shares intent only"); collect works.
- FIXED in 0.16.0 (e5f8309): A worker gave up after "three unanswered waits" although the answer had been sent (the 0.15.2
  batch lead saw the same crossing).
- FIXED in 0.16.0 (58bd487): room_state keeps counting collected and discarded workers as finished.
- FIXED in 0.16.0 (d2f87a2): The discard recovery patch is named with the UTC date.
- FIXED in 0.16.0 (d2f87a2): `archive-signal.test.ts` ("early tar exit while git archive is piping") hung once on CI's Linux
