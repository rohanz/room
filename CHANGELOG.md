...
- Every effective sharing boundary change withdraws file overlays, deletion marks and base text outside the new level or declared/retained paths before reconciliation; registration reconciles even when the reported level already matches.
- Base texts are owned per participant and stored under flat keys, so concurrent first writes from sessions sharing a tag retain every file. Each daemon publishes and withdraws its own entries; eviction and worker retirement also remove a stopped participant's entries atomically with its overlays. Browser File and Merged views read each overlay owner's baseline, with a fallback to legacy shared entries. New clients never write or delete those legacy entries while older clients may still rely on them. Sharing and scope changes still invalidate in-flight reads; superseded asynchronous operations schedule another reconciliation so allowed files are eventually shared.
- Legacy archived-worker repair uses the same retirement cleanup as collection, removing the worktree's retained declared-path record when that checkout remains.
...
- The clone daemon marks a base notice seen for its participant when the notice's commit is already in local HEAD (`git merge-base --is-ancestor <base> HEAD`). It checks existing notices at startup and after HEAD moves, and checks new notices as they arrive on the bus. All delivery paths use the existing seen receipt synchronously; the notice remains in the timeline. Git errors leave it deliverable (`packages/roomd/src/index.ts`).
...
- Failed worker spawn rollback now removes only a checkout and branch created by that spawn. Recreating a vanished checkout on an existing `room/<tag>` branch preserves the branch, its unmerged commits, and existing carry refs if linking or launch fails.
- Leave and shutdown inspect live recorded PIDs for done, failed and dismissed workers, even when ownership cannot be verified, so an unknown live process is reported rather than silently skipped.
