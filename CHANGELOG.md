...
- Worker process recovery now checks the host executable and whole argv session/thread tokens (Linux `/proc` argv; conservative quoted-prompt handling for flattened macOS commands). `ROOM_WORKER_ID` is the preferred exact environment proof, with exact tag/lead environment or verified worktree cwd as fallbacks. Prompt text containing a session id cannot authorize a signal.
...
- Worker PID recovery now requires an exact host session argument, matching `ROOM_TAG` and `ROOM_LEAD` process environment, or the exact worker worktree cwd, alongside the recorded start time. A tag substring in an unrelated Claude or Codex command can no longer qualify it for signalling.
