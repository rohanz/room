...
- Base texts use a shared participant reference set derived from live overlays and deletion marks, replacing process-local published-key tracking. Reconciliation repairs legacy entries and crash leftovers, drops unreferenced text across HEAD changes, and retains text still needed by another participant. Sharing and scope changes invalidate in-flight reads; superseded asynchronous operations schedule another reconciliation so allowed files are eventually shared.
