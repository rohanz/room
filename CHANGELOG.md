...
- Base texts are owned per participant: each daemon publishes and withdraws only its own entries, so concurrent withdrawal and publication cannot erase another person's baseline. Browser File and Merged views read each overlay owner's baseline, with a fallback to legacy shared entries. Legacy entries are collected after their last overlay disappears, including after remote updates. Sharing and scope changes still invalidate in-flight reads; superseded asynchronous operations schedule another reconciliation so allowed files are eventually shared.
